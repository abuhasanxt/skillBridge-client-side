  ...
  return (
    <Card className="max-w-md mx-auto my-8 shadow-lg border border-gray-200 rounded-xl overflow-hidden">
      ...
      <CardHeader className="text-center mt-4">
        ...
        <CardDescription className="text-indigo-600 font-medium">Role:{role}</CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4 px-6 mt-2">
        ...
        <div className="text-center">
          <p className="text-sm text-muted-foreground font-medium">Phone:{phone}</p>
        ...
        <div className="flex justify-between items-center mt-2">
          <div className="flex items-center gap-5">
            <p className="text-sm text-muted-foreground font-medium">Hourly Price</p>
            <p className="text-indigo-600 font-semibold">${tutorProfile?.hourlyPrice ?? "N/A"}</p>
          </div>
            ...
            <div className="flex flex-wrap gap-2 mt-1 justify-end">
              {tutorProfile?.subject?.length ? (
                tutorProfile.subject.map((sub) => (
                  <Badge key={sub} variant="outline" className="text-indigo-700 border-indigo-300">
                    {sub}
                  </Badge>
                ))
              ) : (
      ...
      <CardFooter className="flex justify-center pb-6">
        <Button variant="default" className="hover:bg-indigo-600 hover:text-white transition-colors">
          Edit Profile
        </Button>
      </CardFooter>
    </Card>
